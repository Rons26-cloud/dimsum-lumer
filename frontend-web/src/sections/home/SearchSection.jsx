  ...
  return (
    <section className="px-3 xs:px-4 pt-4">
      <form onSubmit={submit} className="flex items-center gap-2.5 bg-gray-100 rounded-2xl px-3.5 sm:px-4 min-h-12 focus-within:ring-2 focus-within:ring-primary/30 focus-within:bg-white transition-all">
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} type="search" placeholder="Cari dimsum favoritmu…" className="flex-1 bg-transparent text-sm text-gray-700 placeholder:text-gray-400 outline-none min-w-0" aria-label="Cari produk" />
