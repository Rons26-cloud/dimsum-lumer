...
export default function NotificationHeader({ onBack, onReadAll, onDeleteAll, onSettings, unreadCount }) {
  return <header className="sticky top-0 z-30 flex h-[calc(3.5rem+env(safe-area-inset-top))] items-end gap-0.5 border-b border-gray-100 bg-white/95 px-2 pb-2 pt-[env(safe-area-inset-top)] backdrop-blur-md sm:h-[calc(4rem+env(safe-area-inset-top))] sm:px-3 sm:pb-3">
    ...
    <button type="button" onClick={onSettings} className="grid h-10 w-10 place-items-center rounded-full hover:bg-gray-50 active:bg-primary-50 active:text-primary" aria-label="Buka pengaturan notifikasi"><Settings size={18}/></button>
