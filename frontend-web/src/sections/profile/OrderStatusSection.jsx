...
export default function OrderStatusSection({ orders,onSelect }) { const latest=orders.latestOrder;return <section className="rounded-2xl border border-gray-100 bg-white p-2.5 shadow-sm"><div className="mb-1 flex items-center justify-between px-0.5"><h2 className="text-[10px] font-extrabold">Status Pesanan</h2><button onClick={()=>onSelect('all')} className="text-[8px] font-bold text-primary">Lihat semua</button></div><div className="grid grid-cols-5 gap-0.5">{statuses.map(([id,label,Icon])=><OrderStatusCard key={id} icon={id==='shipping'&&latest?((delivery[String(latest.shipping_method||'').toLowerCase()]||{}).Icon||Icon):Icon} label={label} count={orders[id]||0} onClick={()=>onSelect(id)}/>)}</div><OrderTimeline order={latest} onOpen={()=>onSelect(latest?.status||'all')}/></section>; }
