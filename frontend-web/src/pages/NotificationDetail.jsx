...
const statusCopy = {
  pending: {
    eyebrow: "Pesanan berhasil diterima",
    title: "Menunggu Konfirmasi",
    message:
      "Pesananmu sudah masuk dan akan segera diperiksa oleh tim Dimsum Lumer.",
  },
  processing: {
    eyebrow: "Pesanan sedang diproses",
    title: "Sedang Kami Siapkan",
    message:
      "Tim kami sedang menyiapkan pesanan Anda agar segera dapat dikirim.",
  },
  shipping: {
    eyebrow: "Pesanan dalam perjalanan",
    title: "Pesanan Sedang Diantar",
    message:
      "Pesananmu sudah berangkat menuju alamat tujuan. Pantau perjalanannya secara langsung.",
  },
  completed: {
    eyebrow: "Pesanan telah diterima",
    title: "Pesanan Selesai",
    message:
      "Pesanan Anda telah selesai. Terima kasih telah memilih Dimsum Lumer.",
  },
  cancelled: {
    eyebrow: "Informasi pesanan",
    title: "Pesanan Dibatalkan",
    message:
      "Pesanan ini telah dibatalkan. Lihat rincian di bawah untuk informasi selengkapnya.",
  },
};
...
export default function NotificationDetail() {
  ...
  if (!notification)
    return (
      <div className="grid min-h-dvh place-items-center bg-slate-50 px-6 text-center text-sm text-slate-500">
  ...
  const trackable =
    ...
    trackingTypes.has(String(notification.type || "").toLowerCase());
  const status = String(
    order?.status || notification.metadata?.status || "",
  ).toLowerCase();
  const copy = statusCopy[status] || {
    eyebrow: order?.order_code
    ...
    message:
      ...
      notification.body ||
      "Terdapat pembaruan terbaru untuk pesanan Anda.",
  };
  ...
  const displayCopy = isLoginSecurity
    ? {
        ...
        message:
          "Kami mendeteksi akses ke akun Anda dari perangkat yang belum pernah digunakan sebelumnya.",
      }
    : copy;
  ...
  return (
    <div className="mx-auto min-h-dvh max-w-3xl bg-slate-50 pb-8 text-slate-900">
      ...
      <main className="space-y-3 p-3 sm:p-5">
        <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-5 text-white shadow-lg shadow-slate-200">
          ...
          <div className="relative flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <span className="text-[9px] font-bold uppercase tracking-[.18em] text-orange-300">
                {displayCopy.eyebrow}
              </span>
              <h1 className="mt-1 text-xl font-black leading-7">
                {displayCopy.title}
              </h1>
              <div className="mt-1">
                <NotificationTime
          ...
          <div className="relative mt-4 border-t border-white/10 pt-4">
            {orderCode && (
              <span className="inline-flex rounded-full border border-white/10 bg-white/5 px-2.5 py-1 font-mono text-[9px] font-bold tracking-wide text-slate-300">
                {orderCode}
              </span>
            )}
        ...
        {error && (
          <p className="rounded-2xl border border-red-100 bg-red-50 p-3 text-[10px] text-red-600">
            {error}
          </p>
        )}
        {trackable && (
        ...
        {isLoginSecurity && (
          <LoginSecurityDetail metadata={notification.metadata} />
        )}
        {notification.order_id && (
          <NotificationInfo notification={notification} order={order} />
        )}
        <NotificationFooter
          notification={notification}
          order={order}
          onTrack={() => order && navigate(`/lacak-pesanan/${order.id}`)}
        />
      </main>
    </div>
  );
