  ...
  const filtered = useMemo(() => {
    ...
    const result = catalog.filter(
      (product) =>
        ...
        (!availableOnly || Number(product.stock) > 0) &&
        (!term ||
  ...
  return (
    <div className="mx-auto min-h-[calc(100dvh-8.5rem)] max-w-6xl bg-gray-50 md:my-4 md:overflow-hidden md:rounded-3xl">
      <div className="border-b border-gray-100 bg-white px-3 py-3 xs:px-4">
        <div className="mb-3">
          <h1 className="text-lg font-bold">Semua Produk</h1>
          <p className="text-[10px] text-slate-500">
            Temukan pilihan sesuai selera Anda
          </p>
        </div>
        <form
          onSubmit={search}
          className="flex items-center gap-2 bg-gray-100 min-h-11 px-3.5 rounded-xl focus-within:ring-2 focus-within:ring-primary/20"
        >
          ...
          <button
            type="submit"
            className="text-gray-400 active:text-primary"
        ...
        <div className="mt-2.5 flex gap-2">
          <select
            ...
            className="min-h-10 min-w-0 flex-1 rounded-xl border border-gray-200 bg-white px-3 text-xs font-semibold text-gray-700"
          >
          ...
          <button
            ...
            className={`min-h-10 rounded-xl border px-3 text-[10px] font-bold ${availableOnly ? "border-primary bg-primary-50 text-primary" : "border-gray-200 bg-white text-gray-600"}`}
          >
            Stok tersedia
          </button>
        </div>
      ...
      <div className="p-3 pb-24 xs:p-4">
        {products === null ? (
          ...
          <EmptyState
            ...
            description="Coba kata kunci lain atau periksa kembali nanti."
          />
        ) : (
          <>
