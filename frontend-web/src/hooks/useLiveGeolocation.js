...
const explainError = (error) =>
  ({
    ...
    2: "Sinyal GPS tidak tersedia. Coba di area terbuka.",
    3: "Pencarian lokasi terlalu lama. Tekan refresh untuk mencoba lagi.",
  })[error?.code] ||
  error?.message ||
    ...
    const timeoutId = window.setTimeout(
      () =>
        finish(
          reject,
          new Error(
            "Lokasi belum ditemukan. Pastikan GPS aktif, lalu coba kembali.",
          ),
        ),
      15000,
    );
  ...
  const start = useCallback(() => {
    ...
    if (
      ...
      setState((current) => ({
        ...current,
        status: "error",
        error: "GPS memerlukan HTTPS atau localhost.",
      }));
      return;
    }
    if (!navigator.geolocation) {
        ...
        setState((current) =>
          current.coords
            ...
            : {
                ...
                status: "error",
                error:
                  "GPS belum merespons. Pastikan lokasi perangkat aktif lalu tekan refresh.",
              },
        ),
      13000,
    );
