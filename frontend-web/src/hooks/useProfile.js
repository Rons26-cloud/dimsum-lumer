...
export function useProfile() {
  ...
  useEffect(()=>{refresh();if(!user)return undefined;const channel=supabase.channel(`profile-${user.id}-${runtimeId()}`).on("postgres_changes",{event:"*",schema:"public",table:TABLES.PROFILES,filter:`id=eq.${user.id}`},refresh).subscribe();return()=>{supabase.removeChannel(channel);};},[refresh,user]);
