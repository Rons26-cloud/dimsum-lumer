...
export function usePoint(profilePoint = null) {
  ...
  useEffect(()=>{refresh();if(!user)return undefined;const suffix=runtimeId();const orderChannel=supabase.channel(`profile-points-${suffix}`).on('postgres_changes',{event:'*',schema:'public',table:'orders',filter:`user_id=eq.${user.id}`},refresh).on('postgres_changes',{event:'UPDATE',schema:'public',table:'profiles',filter:`id=eq.${user.id}`},refresh).on('postgres_changes',{event:'INSERT',schema:'public',table:'point_history',filter:`user_id=eq.${user.id}`},refresh).subscribe();return()=>{supabase.removeChannel(orderChannel);};},[refresh,user]);
