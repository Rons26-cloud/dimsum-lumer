...
export function usePoint(profilePoint = null) {
  ...
  useEffect(()=>{refresh();if(!user)return undefined;const suffix=runtimeId();const orderChannel=supabase.channel(`profile-orders-${suffix}`).on('postgres_changes',{event:'*',schema:'public',table:'orders',filter:`user_id=eq.${user.id}`},refresh).subscribe();return()=>{supabase.removeChannel(orderChannel);};},[refresh,user]);
