...
export default function Maintenance() {
  ...
  const save = async (event) => {
    ...
    if (invalidSchedule) return setNotice({ type: "error", text: "Waktu selesai harus lebih besar dari waktu mulai." });
    setSaving(true); setNotice(null);
    try {
      const targets = selectedTarget === "both" ? ["frontend-web", "mobile-apk"] : [selectedTarget];
      await Promise.all(targets.map((target) => setMaintenance(target, form.is_active, form.message || DEFAULT_MESSAGE, { start_time: toIso(form.start_time), end_time: toIso(form.end_time) })));
      setNotice({ type: "success", text: `Pengaturan berhasil dikirim realtime ke ${selectedTarget === "both" ? "Frontend Web dan APK" : selectedTarget === "mobile-apk" ? "APK" : "Frontend Web"}.` });
